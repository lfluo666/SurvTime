...
// [[Rcpp::export]]
List NRrepeat(int N,int knot,arma::colvec &facility,arma::colvec &delta,arma::mat &z,arma::mat &b_spline,arma::mat &theta,int number_facility=1,int M_stop=1000,double rate = 0.001){
  int key = 1;
  int p=z.n_cols;
  double tol = 10^(-6);
  
  arma::colvec likelihood_NR_all(M_stop+1);
  likelihood_NR_all(0) = dloglik_likelihood_stratify(knot, facility, delta, z, b_spline, theta,N);
  ...
  while(key<=M_stop)
  {
    ...
    dist.reshape(p,knot);
    double gamma = 1;
    arma::mat theta_temp = theta + dist*gamma;
    
    double likelihood_stratify = dloglik_likelihood_stratify(knot, facility, delta, z, b_spline, theta_temp,N);
    
    while(likelihood_stratify < likelihood_NR_all(key-1)+gamma*rate){
      gamma = gamma / 2.0;
      theta_temp = theta + dist*gamma;
      likelihood_stratify = dloglik_likelihood_gradient(knot,facility,delta,z,b_spline,theta_temp,N);
    }
    theta = theta + dist*gamma;
    
    likelihood_stratify = dloglik_likelihood_stratify(knot, facility, delta, z, b_spline, theta,N);
    likelihood_NR_all(++key) = likelihood_stratify;
    
    if(key >= 2){
      double llk_diff = likelihood_NR_all(key) - likelihood_NR_all(key-1);
      double llk_diff2 = likelihood_NR_all(key) - likelihood_NR_all(1);
  ...
  List result;
  result["theta"]=theta;
  result["likelihood"]=likelihood_NR_all;
  result["GVG"]=temp["GVG"];
